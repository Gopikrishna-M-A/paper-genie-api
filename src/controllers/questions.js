...
export const getAllQuestions = async (req, res) => {
      ...
      try {
        const questions = await Question.find({ userId }); // Fetch questions with the user's ID
...
export const getQuestionsBySubject = async (req, res) => {
    ...
    try {
      const questions = await Question.find({ subject, userId })
      res.status(200).json(questions)
    } catch (error) {
      console.error('Error fetching questions by subject and user ID:', error)
      res.status(500).json({ error: 'Internal server error' })
    }
}   
